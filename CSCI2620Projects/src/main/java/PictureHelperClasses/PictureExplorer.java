...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener
{
  ...
  public PictureExplorer(DigitalPicture picture, boolean expand)
  {
    // set the fields
    this.picture=picture;
    zoomFactor=1;
    ...
    // create the window and set things up
    createWindowWithRotateBar(expand);
  }
  ...
  private void createWindowWithRotateBar(boolean expand)
  {
    // create the picture frame and initialize it
    createAndInitPictureFrame();
    ...
    //create the information panel
    createInfoPanelWithRotateBar(expand);
  ...
  private void setUpNextAndPreviousButtons()
  {
    ...
    Icon prevIcon = new ImageIcon(getClass().getClassLoader().getResource("images/leftArrow.gif"),
                                  "previous index");
    Icon nextIcon = new ImageIcon(getClass().getClassLoader().getResource("images/rightArrow.gif"),
                                  "next index");
  ...
  public JPanel createRotateBar(Font labelFont, boolean expand) {

    // create a location panel
    JPanel rotateSlider = new JPanel();
    rotateSlider.setLayout(new FlowLayout());
    Box hBox = Box.createHorizontalBox();
    ...
    Picture tempPicture = new Picture(this.picture.getBufferedImage());
    ...
    slider.addChangeListener(new ChangeListener() {
      ...
      public void stateChanged(ChangeEvent e) {
        ...
        if (expand) {
          picture = tempPicture.rotateAndExpand(Math.toRadians(tempSlider.getValue()));
        }
        else {
          picture = tempPicture.rotateAndFit(Math.toRadians(tempSlider.getValue()));
        }

        BufferedImage bimg = picture.getBufferedImage();
        imageDisplay.setImage(bimg.getScaledInstance(picture.getWidth(), picture.getHeight(), Image.SCALE_DEFAULT));
        scrollPane.setViewportView(imageDisplay);
        repaint();
      }
    });
  ...
  private void createInfoPanelWithRotateBar(boolean expand)
  {
    // create the info panel and set the layout
    JPanel infoPanel = new JPanel();
    infoPanel.setLayout(new BorderLayout());
    ...
    //create a rotate bar
    JPanel rotateBar = createRotateBar(largerFont, expand);

    // add the panels to the info panel
    infoPanel.add(BorderLayout.NORTH,locationPanel);
    infoPanel.add(BorderLayout.SOUTH,colorInfoPanel);
    infoPanel.add(rotateBar, BorderLayout.CENTER);
  ...
  {
    Picture pix = new Picture("images/arch.png");
    Picture smallArch = pix.scale(.25, .25);
    smallArch.write("smallArch.png"); 
    smallArch.explore();
  }
