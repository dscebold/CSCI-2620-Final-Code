...
public class SimplePicture implements DigitalPicture {
     ...
     * Method to get a two-dimensional array of Pixels for this simple picture
     *
     * This was written by me Drew Scebold, most other functions in here were not
     *
     * @return a two-dimensional array of Pixel objects in column-major order.
     */
    public Pixel[][] getPixels2DColumnMajor() {
        int width = getWidth();
        int height = getHeight();
        Pixel[][] pixelArray = new Pixel[width][height];
        ...
        for(int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                pixelArray[col][row] = new Pixel(this, col, row);
            }
        }
    ...
    public HSVPixel[][] getHSVPixels2DColumnMajor() {
        int width = getWidth();
        int height = getHeight();
        HSVPixel[][] pixelArray = new HSVPixel[width][height];

        //loop through columns from left to right
        for(int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                pixelArray[col][row] = new HSVPixel(new Pixel(this, col, row).getColor());
            }
        }
    ...
    public CMYKPixel[][] getCMYKPixels2DColumnMajor() {
        int width = getWidth();
        int height = getHeight();
        CMYKPixel[][] pixelArray = new CMYKPixel[width][height];

        //loop through columns from left to right
        for(int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                pixelArray[col][row] = new CMYKPixel(new Pixel(this, col, row).getColor());
            }
        }
    ...
    public void updateHSVToRGB(HSVPixel[][] hsvPixels) {
        if(hsvPixels.length != this.getWidth() || hsvPixels[0].length != getHeight()) {
            System.err.println("Pixel Matrices Mismatch");
            return;
        }
        for(int col = 0; col < hsvPixels.length; col++) {
            for (int row = 0; row < hsvPixels[0].length; row++) {
                new Pixel(this, col, row).setColor(hsvPixels[col][row].getRGBColor());
            }
        }
    }

    public void updateCMYKToRGB(CMYKPixel[][] cmykPixels) {
        if(cmykPixels.length != this.getWidth() || cmykPixels[0].length != getHeight()) {
            System.err.println("Pixel Matrices Mismatch");
            return;
        }
        for(int col = 0; col < cmykPixels.length; col++) {
            for (int row = 0; row < cmykPixels[0].length; row++) {
                new Pixel(this, col, row).setColor(cmykPixels[col][row].getRGBColor());
            }
        }
    }
    ...
    public void exploreWithRotate() {
        new PictureExplorer(new SimplePicture(this), true);
    }

    public void exploreWithRotateThatFits() {
        new PictureExplorer(new SimplePicture(this), false);
    }
    ...
    public void loadOrFail(String fileName) throws IOException {
        ...
        File file = null;
        try {
            file = new File(getClass().getClassLoader().getResource(this.fileName).toURI());
    ...
    public void writeOrFail(String fileName) throws IOException {
        ...
        File file = new File("CSCI2620Projects/src/main/resources/" + fileName);
        ...
        write(bufferedImage, extension, file);
    }
